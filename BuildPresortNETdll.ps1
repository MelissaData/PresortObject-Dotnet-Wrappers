# Name:    BuildPresortNETdll
# Purpose: Compile the mdPresortNET.dll wrapper 
...
Write-Host "`n======================= WELCOME TO MELISSA PRESORT OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdPresortNET.dll -platform:AnyCPU -target:library mdPresort_cSharpCode.cs" > $null

if (Test-Path ("mdPresortNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdPresortNET.dll"

  Write-Host "`nmdPresortNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdPresortNET.dll was not created successfully`n"
